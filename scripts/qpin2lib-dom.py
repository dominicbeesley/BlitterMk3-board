...
# 
# DB: 2021-03-25 - taken from https://github.com/puddingpimp/qpin2lib
#
# This version doesn't take the net names from the .pin file instead all pins
# are just labelled with their pin location except for special function pins
...
def fixSignalName(s):
    s = s.replace('[','_').replace(']','')
        #DB - rename special ALTERA signals

    s = re.sub(r'^(~?ALTERA.*)/RESERVED.*', r'\1', s)

    s = re.sub(r'^RESERVED_INPUT.*',r'NOTCONN', s)
...
def drawlist():
    ...
    for (bname,b) in banks.items():
        ...
        l += drawBank(b)
        i = 1
        b.pins.sort(reverse=True,key=pinSortFn)
        for p in b.pins:
